...
//            }
            ...
            if(!move_uploaded_file($_FILES['fastaseq']['tmp_name'], $target_file)){
                ...
                echo 'python process_crap_interface.py ' . $target_file . ' ' . $target_file . '.clean.txt ' . $target_file . '.messy.txt ' . $email . ' -ct ' . $ct . ' -cl ' . $cl . ' -0j ' . $zj . ' -min ' . $min . ' -max ' . $max . ' -fft ' . $fft . ' -ffl ' . $ffl . $ms . '-xs ' . $xs . ' > /dev/null 2>&1 &';
                exec('python process_crap_interface.py ' . $target_file . ' ' . $target_file . '.clean.txt ' . $target_file . '.messy.txt ' . $email . ' -ct ' . $ct . ' -cl ' . $cl . ' -0j ' . $zj . ' -min ' . $min . ' -max ' . $max . ' -fft ' . $fft . ' -ffl ' . $ffl . $ms . '-xs ' . $xs . ' > /dev/null 2>&1 &');
