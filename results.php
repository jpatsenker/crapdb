...
<?php
	...
	if(!$_POST['email']){
        ...
        $next_id = get_next_id();
        $target_fname = explode(".", $_FILES['fastaseq']['tmp_name'])[0] . $next_id;
        ...
        if(!move_uploaded_file($_FILES['fastaseq']['tmp_name'], $target_file)){
            ...
            echo 'python run_cra_interface.py ' . $target_file . ' ' . $target_file . '.clean.txt ' . $target_file . '.messy.txt ' . $email . ' -ct ' . $ct . ' -cl ' . $cl . ' -0j ' . $zj . ' -min ' . $min . ' -max ' . $max . $ms . ' -xs ' . $xs . $dComp . $dLen . $dRed . $dFf .' > superlog 2>&1 &';
            exec('python run_cra_interface.py ' . $target_file . ' ' . $target_file . '.clean.txt ' . $target_file . '.messy.txt ' . $email . ' -ct ' . $ct . ' -cl ' . $cl . ' -0j ' . $zj . ' -min ' . $min . ' -max ' . $max . $ms . ' -xs ' . $xs . $dComp . $dLen . $dRed . $dFf .' > superlog 2>&1 &');
